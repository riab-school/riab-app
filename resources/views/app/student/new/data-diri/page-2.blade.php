...
<div class="row">
    ...
    <div class="col-md-10">
        @if(auth()->user()->myDetail && $origin !== NULL && $origin->is_rejected !== NULL && $origin->is_rejected)
        ...
        <div class="card">
            ...
            <div class="card-body">
                @if($isCompletedOrigin)
                    <div class="alert alert-danger" role="alert">
                        Data asal sekolah Anda sudah diverifikasi, anda dapat melakukan perubahan data dengan menghubungi admin atau panitia.
                    </div>
                @endif

                <form action="{{ route('student.new.data-diri.store-page-2') }}" method="POST" onsubmit="return processData(this)">
                    @if(!$isCompletedOrigin)
                    @csrf
                    @endif
