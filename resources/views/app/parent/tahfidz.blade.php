...
@section('content')
    <div class="container">
        <div class="section-heading d-flex align-items-center justify-content-between rtl-flex-d-row-r">
            <h6>Total Catatan Hafalan dan Setoran
                <span id="totalCount"></span>
            </h6>
        </div>
        <div class="row g-2" id="renderRiwayat">
...
<script>
    ...
    function loadRiwayat() {
        ...
        isLoading = true;
        $.ajax({
            url: "{{ route('parent.tahfidz.history') }}",
            type: "GET",
            data: { page: page },
            dataType: "json",
                                    ...
                                    <div class="product-thumbnail-side">
                                        ${item.evidence ? `
                                            <a class="product-thumbnail shadow-sm d-block" href="javascript:void(0);">
                                                <img src="${item.evidence}" data-src="${item.evidence}" alt="icon_quran" class="img-preview img-fluid w-80">
                                            </a>
                                        ` : `
                                            <a class="product-thumbnail shadow-sm d-block" href="javascript:void(0);">
                                                <img src="{{ asset('mobile-assets/img/icons/quran.png') }}" alt="icon_quran" class="img-fluid w-80">
                                            </a>
                                        `}
                                    </div>
                                    <div class="product-description">
                                        <div class="product-title d-block">${item.surah}</div>
                                        <div class="d-block">Ayat ${item.from_ayat} s/d ${item.to_ayat}</div>
                                        <div class="d-block">${item.createdAt}</div>
    ...
    $(document).on('click', '#btnModalDetail', function() {
        let id = $(this).data('id');
        $.ajax({
            url: "{{ route('parent.tahfidz.detail') }}",
            type: "GET",
            data: { id: id },
            dataType: "json",
            success: function(res) {
                let html = `
                    <div class="row">
                        <div class="col-12">
                            <h6>Surat :</h6>
                            <p class="px-3">${res.data.surah}</p>
                        </div>
                        <div class="col-12">
                            <h6>Ayat :</h6>
                            <p class="px-3">${res.data.from_ayat} s/d ${res.data.to_ayat}</p>
                        </div>
                        <div class="col-12">
                            <h6>Point Tahsin :</h6>
                            <p class="px-3">${res.data.point_tahsin}</p>
                        </div>
                        <div class="col-12">
                            <h6>Point Tahfidz :</h6>
                            <p class="px-3">${res.data.point_tahfidz}</p>
                        </div>
                        <div class="col-12">
                            <h6>Keterangan Juz & Halaman :</h6>
                            <p class="px-3">${res.data.ket_juz_halaman ?? '-'}</p>
                        </div>
                        <div class="col-12">
                            <h6>Catatan Penyimak :</h6>
                            <p class="px-3">${res.data.note}</p>
                        </div>
                        <div class="col-12">
                            <h6>Di Simak Oleh :</h6>
                            <p class="px-3">${res.data.tasmik_by.staff_detail.name}</p>
                        </div>
                        <div class="col-12">
                            <h6>Tanggal Setoran :</h6>
