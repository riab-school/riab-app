...
<div class="row">
    <div class="col-md-4">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('staff.prestasi.handle.create') }}" method="POST" onsubmit="return processData(this)" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="d-none" id="input-section">
                        ...
                        <div class="form-group">
                            <label for="detail">Keterangan Prestasi</label>
                            <textarea class="form-control @error('detail') is-invalid @enderror" id="detail" name="detail" rows="3" placeholder="Keterangan Prestasi" required></textarea>
                            @error('detail')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        ...
                        <div class="form-group">
                            <label for="evidence">Bukti / Sertifikat <span class="text-danger">(Tidak Wajib)</span></label>
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <form action="{{ route('staff.prestasi.laporan.handle') }}" method="POST" onsubmit="processData(this);" id="form-report" class="d-none">
                    @csrf
        ...
        $(document).ready(function() {
            $('#search-button').on('click', function() {
                ...
                if (nisOrNisn.length >= 3) {
                    $.ajax({
                        url: '{{ route('staff.prestasi.search') }}',
                        type: 'GET',
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                $.each(response.data.student_achievement_history, function(index, item) {
                                    tableBody.append(`
                                        <tr>
                                            <td>${index + 1}</td>
                                            <td>${item.detail}</td>
