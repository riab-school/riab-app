    ...
    public function showDashboard(Request $request)
    {
        if($request->ajax()){
            $data = [
                ...
                'top_violation_count'   => StudentsViolation::select('user_id', \DB::raw('count(*) as total'))->groupBy('user_id')->with('detail.studentDetail')->orderBy('total', 'desc')->limit(10)->get(),
                'top_month_violation'   => StudentsViolation::whereBetween('created_at', [now()->startOfMonth(), now()->endOfMonth()])
