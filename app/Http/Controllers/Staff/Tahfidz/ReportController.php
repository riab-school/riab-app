...
class ReportController extends Controller
{
    ...
    {
        return view('app.staff.tahfidz-hafalan.report.index');
    }
    ...
    public function handleReportPrint(Request $request)
    {
        ...
        try {
            ...
            if($request->report_by == 'date'){
                $data['achievements'] = StudentsMemorization::whereBetween('created_at', [$request->from_date.' 00:00:00', $request->to_date.' 23:59:59'])->get();
                $data['from_date'] = dateIndo($request->from_date);
                $data['to_date'] = dateIndo($request->to_date);
                $data['report_by'] = 'date';
                return view('app.staff.tahfidz-hafalan.report.print-report', $data);
            } elseif($request->report_by == 'nis_nisn'){
                $data['achievements'] = StudentsMemorization::where(function ($query) use ($request) {
